...
import {
  Activity, Map as MapIcon, Package, AlertTriangle, Gauge,
  MapPin, Bell, TrendingUp, RefreshCw,
} from 'lucide-react'
...
export default function DashboardGases() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-lg font-semibold">Dashboard Inventario Gases</h2>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={<Package className="w-5 h-5" />} label="Total de Tubos" value={stats.total.toString()} accent="from-orange-500 to-red-600" />
        <KpiCard icon={<Gauge className="w-5 h-5" />} label="Capacidad Total" value={`${totalCapacidad} L`} accent="from-emerald-500 to-teal-600" />
        <KpiCard icon={<MapPin className="w-5 h-5" />} label="Ubicaciones" value={stats.porUbicacion.length.toString()} accent="from-sky-500 to-blue-600" />
        <KpiCard icon={<AlertTriangle className="w-5 h-5" />} label="Vencidos" value={stats.enAlertaVencimiento.length.toString()} accent="from-red-500 to-rose-600" />
        <KpiCard icon={<Bell className="w-5 h-5" />} label="Total Alertas" value={stats.totalAlertas.toString()} accent="from-amber-500 to-orange-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Estado actual de los tubos en planta y en distribución</CardDescription>
          </CardHeader>
                ...
                <div key={est} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-700">{ESTADO_LABELS[est]}</span>
                    </div>
                    <span className="text-slate-900 font-semibold">{cantidad} <span className="text-xs text-slate-400 font-normal">({pct.toFixed(0)}%)</span></span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full ${ESTADO_COLORS[est]} rounded-full transition-all`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
              )
            })}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Cantidad de tubos y capacidad total por tipo de gas</CardDescription>
          </CardHeader>
          <CardContent>
            <ScrollArea className="h-[260px] pr-4">
              <div className="space-y-3">
                {stats.porGas.sort((a, b) => b.cantidad - a.cantidad).map((g) => (
                  <div key={g.gas.id} className="flex items-center justify-between gap-3 p-2 rounded-lg hover:bg-slate-50">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="w-4 h-4 rounded-full flex-shrink-0 border-2 border-white shadow" style={{ background: g.gas.colorHex }} />
                      <div className="min-w-0">
                        <div className="font-medium text-sm text-slate-800 truncate">{g.gas.nombre}</div>
                        <div className="text-xs text-slate-500">{g.gas.codigo} · {g.capacidadTotal} L</div>
                      </div>
                    </div>
                    <Badge variant="secondary" className="font-bold tabular-nums">{g.cantidad}</Badge>
                  </div>
                ))}
              </div>
            </ScrollArea>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Cantidad de tubos por ciudad (top 10)</CardDescription>
          </CardHeader>
          <CardContent>
            <ScrollArea className="h-[260px] pr-4">
                    ...
                    <div key={u.ubicacion} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-slate-700">{u.ubicacion}</span>
                          <span className="text-xs text-slate-400">· {u.provincia}</span>
                        </div>
                        <span className="font-semibold text-slate-900 tabular-nums">{u.cantidad}</span>
                      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Tubos con prueba hidrostática vencida</CardDescription>
          </CardHeader>
          <CardContent>
            {stats.enAlertaVencimiento.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-xs text-slate-500">Todos los tubos tienen su inspección al día</p>
              </div>
            ) : (
              <ScrollArea className="h-[260px] pr-4">
                <div className="space-y-2">
                  {stats.enAlertaVencimiento.map((c) => (
                    <div key={c.id} className="flex items-center gap-3 p-2 rounded-lg border border-red-100 bg-red-50/50">
                      <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: c.gas.colorHex }} />
                      <div className="flex-1 min-w-0">
                        <div className="font-mono text-xs font-semibold text-slate-800 truncate">{c.numeroSerie}</div>
                        <div className="text-xs text-slate-500">{c.gas.nombre} · {c.capacidadLitros}L · {c.ubicacionNombre}</div>
                      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Cantidad de tubos según su capacidad en litros</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {stats.porCapacidad.sort((a, b) => a.capacidad - b.capacidad).map((c) => (
              <div key={c.capacidad} className="text-center p-4 rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white">
                <div className="text-2xl font-bold text-slate-900">{c.capacidad}</div>
                <div className="text-xs text-slate-500 mb-2">litros</div>
                <Badge variant="secondary" className="font-bold">{c.cantidad} tubos</Badge>
              </div>
            ))}
          </div>
        </CardContent>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="w-36">
                <select value={filtroGas} onChange={(e) => setFiltroGas(e.target.value)}
                  ...
                  {gases.map((g) => (<option key={g.id} value={g.id}>{g.nombre}</option>))}
                </select>
              </div>
              <div className="w-36">
