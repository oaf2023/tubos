...
export default function ComprobantesTab() {
  ...
  return <div className="space-y-4">
    ...
    <Dialog open={dialog} onOpenChange={setDialog}><DialogContent className="max-w-6xl max-h-[92vh] overflow-y-auto"><DialogHeader><DialogTitle>Nuevo Comprobante</DialogTitle></DialogHeader><div className="grid grid-cols-1 lg:grid-cols-2 gap-5"><div className="space-y-4">
      ...
      <div><Label>Observaciones / ML / MP</Label><Textarea rows={5} value={form.observaciones} onChange={e => setForm((f: any) => ({ ...f, observaciones: e.target.value }))} placeholder="Venta #, datos de envío, comprador, Mercado Pago, etc." /></div>
    ...
    </div><div className="space-y-3"><DocumentoPreview doc={draftPreview} config={config} /></div></div><DialogFooter><Button variant="outline" onClick={() => setDialog(false)}>Cancelar</Button><Button onClick={save}><Save className="w-4 h-4 mr-1" />Guardar</Button></DialogFooter></DialogContent></Dialog>
