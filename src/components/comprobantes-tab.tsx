...
export default function ComprobantesTab() {
  ...
  const [entregaPreview, setEntregaPreview] = useState<Doc | null>(null)
  ...
  async function load() {
    ...
    try {
      ...
      let docs = await d.json()
      if (filtro === 'all' || filtro === 'REMITOS') {
        const rRes = await fetch('/api/remitos')
        const rData = await rRes.json().catch(() => ({}))
        const entrega = (Array.isArray(rData) ? rData : rData?.data || []).map((r: any) => ({
          id: `rem-${r.id}`,
          origen: 'entrega',
          _remito: r,
          ...
          fecha: r.fecha,
          clienteNombre: r.cliente || 'Sin asignar',
          moneda: '—',
          total: null,
          estado: r.estado,
          fiscal: false,
        ...
        if (search) {
          ...
          docs = docs.filter((x: any) => x.origen !== 'entrega' || `${x.clienteNombre || ''} ${x.numeroFormateado || ''}`.toLowerCase().includes(q))
        }
      }
      setDocs(docs)
  ...
  async function setCliente(id: string) {
    if (id === 'CONSUMIDOR_FINAL') {
      setForm((f: any) => ({
        ...f, clienteId: '', consumidorFinal: true,
        clienteNombre: 'Consumidor Final', clienteCodigo: '', clienteDocumentoNumero: '',
        clienteDocumentoTipo: 'D.N.I.', clienteCondicionIva: 'Consumidor Final',
  ...
  return <div className="space-y-4">
    ...
    {loading ? <div className="space-y-2">{[1,2,3].map(i => <Skeleton key={i} className="h-14" />)}</div> : <div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>Tipo</TableHead><TableHead>Número</TableHead><TableHead>Fecha</TableHead><TableHead>Cliente</TableHead><TableHead>Origen</TableHead><TableHead>Moneda</TableHead><TableHead className="text-right">Total</TableHead><TableHead>Estado</TableHead><TableHead className="text-right">Acciones</TableHead></TableRow></TableHeader><TableBody>{docs.map(d => { const esEntrega = d.origen === 'entrega'; return <TableRow key={d.id} className={esEntrega ? 'bg-orange-50/70' : ''}><TableCell>{esEntrega ? <Badge variant="outline" className="border-orange-300 text-orange-700 bg-orange-50"><Cylinder className="w-3 h-3 mr-1" />Remito de tubos</Badge> : d.tipoDocumento === 'REMITO' ? <Badge variant="outline"><FileText className="w-3 h-3 mr-1" />REMITO X</Badge> : <Badge variant="outline">{d.tipoDocumento} {d.letra}</Badge>}</TableCell><TableCell className="font-mono text-xs">{d.numeroFormateado}</TableCell><TableCell>{new Date(d.fecha).toLocaleDateString('es-AR')}</TableCell><TableCell>{d.clienteNombre}</TableCell><TableCell>{esEntrega ? 'Entrega de tubos' : d.origen}</TableCell><TableCell>{d.moneda}</TableCell><TableCell className="text-right font-mono">{esEntrega ? '—' : fmt(d.total)}</TableCell><TableCell><Badge className={esEntrega ? (d.estado === 'FIRMADO' ? 'bg-emerald-600' : d.estado === 'COMPLETADO' ? 'bg-amber-500' : d.estado === 'PARCIAL' ? 'bg-slate-500' : '') : (d.estado === 'AUTORIZADO' ? 'bg-emerald-600' : d.estado === 'RECHAZADO' ? 'bg-red-600' : d.estado === 'EN_AUTORIZACION' ? 'bg-amber-500' : '')}>{d.estado}</Badge></TableCell><TableCell className="text-right whitespace-nowrap">
              ...
              <Button variant="ghost" size="icon" onClick={() => esEntrega ? setEntregaPreview(d) : setPreview(d)}><Eye className="w-4 h-4" /></Button>
              {!esEntrega && <Button variant="ghost" size="icon" className="text-red-500" onClick={async () => { if(confirm('¿Eliminar comprobante?')) { await fetch(`/api/comprobantes/${d.id}`, { method: 'DELETE' }); load() } }}><Trash2 className="w-4 h-4" /></Button>}
            </TableCell></TableRow> })}</TableBody></Table></div>}

    <Dialog open={dialog} onOpenChange={setDialog}><DialogContent className="w-[95vw] !max-w-[1400px] max-h-[94vh] overflow-y-auto"><DialogHeader><DialogTitle>Nuevo Comprobante</DialogTitle></DialogHeader><div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900"><strong>Cómo usar esta pantalla:</strong> cargá los datos en la columna izquierda. La hoja de la derecha es solo una vista previa visual del comprobante; el número definitivo y los totales fiscales se asignan al presionar <strong>Guardar</strong>.</div><div className="grid grid-cols-1 2xl:grid-cols-[minmax(480px,0.8fr)_minmax(620px,1.2fr)] gap-5"><div className="min-w-0 space-y-4"><div className="rounded-lg border bg-slate-50 p-3"><div className="text-sm font-semibold text-slate-800">1. Datos principales</div><div className="text-xs text-slate-500">Elegí tipo, fecha, moneda, cliente y condición de venta.</div></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"><div><Label>Tipo</Label><select className="w-full border rounded px-3 py-2 text-sm" value={form.tipo} onChange={e => setForm((f: any) => ({ ...f, tipo: e.target.value }))}>{TIPO_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div><div><Label>Fecha</Label><Input type="date" value={form.fecha} onChange={e => setForm((f: any) => ({ ...f, fecha: e.target.value }))} /></div><div><Label>Estado</Label><input type="text" className="w-full border rounded px-3 py-2 text-sm bg-slate-100 text-slate-500" value="BORRADOR" disabled /></div></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"><div><Label>Moneda</Label><select className="w-full border rounded px-3 py-2 text-sm" value={form.moneda} onChange={e => setForm((f: any) => ({ ...f, moneda: e.target.value }))}><option value="ARS">ARS</option><option value="USD">USD</option></select></div><div><Label>Tipo cambio</Label><Input type="number" step="0.01" value={form.tipoCambio} onChange={e => setForm((f: any) => ({ ...f, tipoCambio: parseFloat(e.target.value) || 1 }))} /></div><div><Label>Condición venta</Label><Input value={form.condicionVenta} onChange={e => setForm((f: any) => ({ ...f, condicionVenta: e.target.value }))} /></div></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><div><Label className="text-xs">Saldo anterior</Label><Input type="number" className="h-7 text-sm" value={form.saldoAnterior} onChange={e => setForm((f: any) => ({ ...f, saldoAnterior: parseFloat(e.target.value) || 0 }))} /></div></div>
    ...
    </div><div className="min-w-0 space-y-3 2xl:sticky 2xl:top-0 2xl:self-start"><div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><div className="font-semibold">Vista previa del comprobante</div><div className="text-xs">Esta hoja muestra cómo se verá impreso. No se guarda hasta presionar Guardar. El número real sale del numerador configurado en Tablas.</div></div><div className="overflow-x-auto rounded-lg border bg-slate-100 p-3 shadow-sm"><div className="w-full min-w-0"><DocumentoPreview doc={draftPreview} config={config} /></div></div></div></div><DialogFooter><Button variant="outline" onClick={() => setDialog(false)}>Cancelar</Button><Button onClick={save}><Save className="w-4 h-4 mr-1" />Guardar comprobante</Button></DialogFooter></DialogContent></Dialog>
    <Dialog open={!!preview} onOpenChange={o => { if(!o) setPreview(null) }}><DialogContent className="w-[95vw] !max-w-[1000px] max-h-[94vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Printer className="w-5 h-5" />Vista previa</DialogTitle></DialogHeader>{preview && <div className="space-y-3"><div className="flex items-center gap-2 flex-wrap"><Badge className={preview.estado === 'AUTORIZADO' ? 'bg-emerald-600' : preview.estado === 'RECHAZADO' ? 'bg-red-600' : preview.estado === 'EN_AUTORIZACION' ? 'bg-amber-500' : ''}>{preview.estado}</Badge>{preview.fiscal && preview.estado === 'BORRADOR' && <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-xs" onClick={() => autorizarComprobante(preview.id)} disabled={autorizando === preview.id}>{autorizando === preview.id ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <ShieldCheck className="w-4 h-4 mr-1" />}Autorizar contra ARCA</Button>}{preview.estado === 'AUTORIZADO' && <Badge className="bg-emerald-600">✓ Autorizado</Badge>}{preview.fiscal && !preview.cae && preview.estado === 'BORRADOR' && <Button size="sm" variant="outline" className="text-xs" onClick={async () => { const cae = prompt('Ingrese CAE manual:'); if (!cae) return; try { const res = await fetch(`/api/comprobantes/${preview.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cae }) }); if (res.ok) { toast({ title: 'CAE manual guardado' }); load(); const updated = await res.json(); setPreview(updated) } else toast({ title: 'Error al guardar CAE', variant: 'destructive' }) } catch { toast({ title: 'Error', variant: 'destructive' }) } }}>CAE manual</Button>}</div><div className="overflow-x-auto"><div className="w-full min-w-0"><DocumentoPreview doc={preview} config={config} comprobanteAsociado={previewAsociado} /></div></div></div>}</DialogContent></Dialog>
    <Dialog open={!!entregaPreview} onOpenChange={o => { if(!o) setEntregaPreview(null) }}><DialogContent className="!w-[96vw] !max-w-[900px] max-h-[94vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><Cylinder className="w-5 h-5 text-orange-500" />Remito de tubos {entregaPreview?.numeroFormateado}</DialogTitle></DialogHeader>{entregaPreview && (() => { const r = entregaPreview._remito; const items = r.items || []; const desc = items.filter((i: any) => i.descargado).length; return <div className="space-y-3"><div className="text-sm space-y-1"><p><span className="text-slate-500">Cliente:</span> <b>{r.cliente || 'Sin asignar'}</b></p><p><span className="text-slate-500">Fecha:</span> {new Date(r.fecha).toLocaleDateString('es-AR')}</p>{r.tecnico && <p><span className="text-slate-500">Conductor:</span> {r.tecnico}</p>}<p><span className="text-slate-500">Estado:</span> <Badge className={r.estado === 'FIRMADO' ? 'bg-emerald-600' : r.estado === 'COMPLETADO' ? 'bg-amber-500' : r.estado === 'PARCIAL' ? 'bg-slate-500' : ''}>{r.estado}</Badge></p><p><span className="text-slate-500">Tubos descargados:</span> {desc}/{items.length}{r.firmaNombreCliente && <> · <span className="text-slate-500">Firmado por:</span> <b>{r.firmaNombreCliente}</b> ({r.firmaMetodo || '—'})</>}</p></div><div className="border rounded-lg overflow-hidden"><Table><TableHeader><TableRow><TableHead>Serie</TableHead><TableHead>Gas</TableHead><TableHead className="text-center">Descargado</TableHead><TableHead>Novedad</TableHead></TableRow></TableHeader><TableBody>{items.map((it: any, i: number) => <TableRow key={i}><TableCell className="font-mono text-xs">{it.numeroSerie || `Gas ${it.gasCodigo} (x${it.cantidad})`}</TableCell><TableCell>{it.gasCodigo}</TableCell><TableCell className="text-center">{it.descargado ? <span className="text-emerald-600 font-semibold">Sí</span> : <span className="text-amber-600 font-semibold">No</span>}</TableCell><TableCell className="text-xs">{it.novedad || '—'}</TableCell></TableRow>)}</TableBody></Table></div>{r.firmaCliente && <img src={r.firmaCliente} alt="Firma" className="max-h-32 bg-white border rounded-lg p-2" />}{r.firmaToken && <p className="text-xs"><span className="text-slate-500">Enlace de firma:</span> <a className="text-orange-600 underline break-all" href={`/firmar/${r.firmaToken}`}>{window.location.origin}/firmar/{r.firmaToken}</a></p>}</div> })()}</DialogContent></Dialog>
  </div>
}
