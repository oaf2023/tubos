...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    await db.pedidoLectura.update({
      ...
      data: {
        estado: 'RECHAZADO',
        observacion: motivo ? `${order.observacion ? order.observacion + '\n' : ''}RECHAZADO: ${motivo}` : order.observacion,
      },
    })
    ...
    await logAudit({
      ...
      detalle: { estadoAnterior: order.estado, estadoNuevo: 'RECHAZADO', motivo },
    })

    return NextResponse.json({ success: true, estado: 'RECHAZADO' })
  } catch (e) {
    console.error('POST /api/internal/orders/[orderId]/reject', e)
