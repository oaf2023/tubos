...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    await logAudit({
      ...
      detalle: { estadoAnterior: order.estado, estadoNuevo: 'CERRADO' },
    })

    return NextResponse.json({ success: true, estado: 'CERRADO' })
  } catch (e) {
    console.error('POST /api/internal/orders/[orderId]/close', e)
