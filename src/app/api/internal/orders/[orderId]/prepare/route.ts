...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    await logAudit({
      ...
      detalle: { estadoAnterior: order.estado, estadoNuevo: 'EN_PREPARACION' },
    })

    return NextResponse.json({ success: true, estado: 'EN_PREPARACION' })
  } catch (e) {
    console.error('POST /api/internal/orders/[orderId]/prepare', e)
    return NextResponse.json({ error: 'Error al preparar pedido' }, { status: 500 })
  }
}
